...
app.get("/api/gifts/:idGift", async (req, res) => {
  try {
    ...
    if (!categories || categories.length === 0) {
      return res.status(404).json({ error: "No categories found for this gift" });
    }
    ...
    // Check if products have already been selected and saved in the cart
    ...
    const selectedProducts = await fetchDataFromTableCondition("cart", "PresentId", presentId);
